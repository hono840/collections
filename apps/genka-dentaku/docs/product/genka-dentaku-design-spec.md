...
| タイポ | 日本語ゴシック（システムファースト、任意で Noto Sans JP 自己ホスト）＋**主役数字は等幅（tabular）**。ヒーロー数字は任意で **IBM Plex Mono**（電卓／レシート／台帳の精密感） | 個性は「奇抜なフォント」ではなく「数字とステータス体系」から出す。厨房での可読性・低ロードを最優先 |
...
--font-num:  "IBM Plex Mono", "Roboto Mono", ui-monospace,
             "SFMono-Regular", "Hiragino Kaku Gothic ProN", monospace;
```
...
- **数字（署名要素）**: ヒーロー数値（ダッシュボードKPIヒーロー、レシピエディタのライブ合計、シミュのbefore/after）は `--font-num`（**IBM Plex Mono** 推奨）で「電卓／レシート／台帳」の精密感を出す。等幅なので桁が自然に揃う。**推奨採用**（記憶に残る一点の中核）。導入コストを避けたい場合は `--font-sans` + `tabular-nums` にフォールバック可。
...
- **プラン切替（PlanToggle）**: 月額 / 年額 の SegmentedControl。年額側に **「2ヶ月分お得」バッジ**（¥9,800/年 = 実質¥817/月）。
...
- 危険閾値超過メニューを、ダッシュボード上部に集約バナー（危険色＋⬢アイコン＋「危険な原価率のメニューが◯件」＋該当へジャンプ）。Freeではロック（ゴールド鍵＋UpgradePrompt）。
...
1. **`apps/genka-dentaku/src/styles/theme.css`（新規）** に本書 §2 の CSS カスタムプロパティを **`:root` で全定義**。Tailwind v4 の `@theme` にマップして Tailwind ユーティリティから参照可能にする。例（抜粋・値は §2 準拠）:
   ...
   @theme {
     ...
     --font-num: "IBM Plex Mono","Roboto Mono",ui-monospace,monospace;
   }
   ```
...
5. **フォント**: 既定はシステムファースト（追加ロードなし）。ブランド統一で Noto Sans JP を採用する場合は **self-host + サブセット + `font-display: swap`**。ヒーロー数値の IBM Plex Mono は **数字・`¥`・`%`・区切りのサブセット**のみ self-host（Cloudflare Pages 静的配信、CLS/ロード最小化）。
...
3. **フォント**: 厨房での可読性・低ロードを最優先しシステムファーストの日本語ゴシック。個性は奇抜なフォントでなく**等幅の主役数字**から出す。ヒーロー数値に IBM Plex Mono（電卓/レシート/台帳の精密感）を推奨し、ブランド名「電卓」の比喩を数字の見た目で回収。
