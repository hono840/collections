...
2. **CPO エージェント**: 「docs/product/ 内の受入条件に対して最新の実装を検証せよ。
   ...
   デザイン設計書（{app-name}-design-spec.md）がある場合は ui-ux-designer に
   ビジュアル一貫性（トークン整合・装飾・余白・A11yコントラスト）のレビューも委譲すること。
   あなた自身はテストを実行せず、サブエージェントの成果物をレビュー・改善指示すること」
