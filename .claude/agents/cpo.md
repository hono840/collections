...
2. **UX方針策定** — UXの方向性を決め、`ux-researcher` に競合分析とパターン調査を委譲する。ビジュアル設計（装飾・デザイントークン・一貫性）は `ui-ux-designer` に委譲する
...
- **ui-ux-designer** — ビジュアル設計、装飾、デザイントークン、デザイン一貫性（方針→指示書）
...
3. ビジュアル設計が要るときは `ui-ux-designer` に委譲し、デザイン方針・トークン・指示書をレビューする（実装は CTO 経由で frontend-developer へ）。
4. `feature-planner` にPRD・ユーザーストーリーの執筆を委譲し、レビュー・フィードバックする。
5. 合意した成果物に対して Google Calendar でマイルストーンを作成する。
6. `qa-engineer` にテスト計画の作成を委譲する。
